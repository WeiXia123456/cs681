...
public class Client {
    public static void main(String[] args) {
       ...
       List<BasketballPlayer> l1 =  playerList.stream().sorted((p1, p2) -> p1.getAge() - p2.getAge()).collect(Collectors.toList());
       System.out.println("sort by age");
       l1.forEach(p -> System.out.println("Name:" + p.getName() + ", age:" + p.getAge()));

        List<BasketballPlayer> l2 =  playerList.stream().sorted((p1, p2) -> p1.getSalary() - p2.getSalary()).collect(Collectors.toList());
        System.out.println("sort by salary");
        l2.forEach(p -> System.out.println("Name:" + p.getName() + ", salary:$" + p.getSalary() + "M"));

        Integer minimumHeight = playerList.stream().map(p -> p.getHeight()).reduce(0, (result, height) ->{
            if (result == 0) {
                return height;
            }
            if (result > height) {
        ...
        Integer maxSalary = playerList.stream().map(p -> p.getSalary()).reduce(0, (result, salary) ->{
            if (result == 0) {
                return salary;
            }
            if (result < salary) {
        ...
        long num = playerList.stream().count();
        System.out.print("number of players is " + num);
    }
}
